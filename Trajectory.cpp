...
int Trajectory::clampIndex(unsigned int index){
	if(index < 0)
		return 0;
...
Trajectory::Trajectory(vector<PoseKey*> cps){
	controlPoints = cps;
	splineType = catmullRom;
	cycleType = pendulum;
	init();
...
Trajectory::Trajectory(cubicSpline spline){
	//generateGeneralControlPoints();
	splineType = spline;
	cycleType = pendulum;
	init();
}
...
void Trajectory::generateGeneralControlPoints(){
...
virtual Pose& Trajectory::update(){
